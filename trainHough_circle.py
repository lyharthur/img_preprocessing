...
for i in range(len(imglistO)):
    ...
    edgestmp = cv2.Canny(blur,50,180)
    ...
    circles1 = cv2.HoughCircles(edgestmp,cv2.HOUGH_GRADIENT,1,5,param1=140,param2=30,minRadius=10,maxRadius=70)

    if circles1 != None:
        circles = circles1[0,:,:]#提取为二维
        circles = np.uint16(np.around(circles))#四舍五入，取整
        for i in circles[:]: 
            cv2.circle(imgO,(i[0],i[1]),i[2]+1,(255,0,0),3)#画圆
            cv2.circle(imgO,(i[0],i[1]),2,(255,0,255),1)#画圆心
            cv2.circle(edgestmp,(i[0],i[1]),i[2]+1,0,3)#画圆
