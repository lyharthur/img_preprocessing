...
imglistR = glob( "1. STI_ET Layer(preprocessing)/**/OK/**REF.tif" )
imglistD = glob( "1. STI_ET Layer(preprocessing)/**/OK/**DEF.tif" )
...
for i in range(len(imglistD)):
    ...
    imgnew = cv2.Canny(imgnew,90,180)
    img2 = cv2.Canny(img2,90,180)
    ...
    lines = cv2.HoughLinesP(imgnew,1,np.pi/180,60,minLineLength=20,maxLineGap=5)
    if lines != None:
        lines1 = lines[:,0,:]
        for x1,y1,x2,y2 in lines1[:]:
            if (x2 - x1 <= 7 and x2 - x1 >= 0)  or (y1 - y2 <= 7 and y1 - y2 >= 0):
...
##    print(filenameN)
    ...
    filename = imglistD[i]
    filename = filename.replace('DEF','NEW')
