...
for i in range(len(imglistD)):
   ...
   #hash
   image1 = Image.open(imglistR[i]).convert('1')
   image2 = Image.open(imglistD[i]).convert('1')
   hash_value = imagehash.phash(image1) - imagehash.phash(image2)
   print(hash_value)
   ...
   #shift
   M = np.float32([[1,0,-x1st],[0,1,-y1st]])
   imgnew = cv2.warpAffine(img1,M,(480,480))
   ...
   imgnew = cv2.GaussianBlur(imgnew,(7,7),0)
   ...
   cv2.imwrite('123.jpg',imgnew)
   cv2.imwrite('222.jpg',img2)
